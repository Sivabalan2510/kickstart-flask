...
sudo docker compose -f docker/compose/crm.yml --env-file docker/compose/STG.env -p sloopstash-stg-crm up -d --scale app=3 --scale nginx=2'''
    ...
    stage('CD: Execute deployment') {
      steps {
        ...
        ansiblePlaybook(credentialsId: 'ansible-node-ssh', playbook: 'kickstart-ansible/playbook/nginx.yml', inventory: 'kickstart-ansible/inventory/stg', limit: 'sloopstash-stg-crm-nginx-1', tags: 'setup,update,configure,stop,start')
